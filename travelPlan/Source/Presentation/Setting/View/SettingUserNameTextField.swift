...
final class SettingUserNameTextField: UITextField {
  typealias State = NicknameValidateState
  ...
  @Published var textState: State = .default {
    didSet {
      ...
      let noticeIconPath = noticeIconPath(from: textState)
      if noticeIconPath == "" {
        noticeIcon.image = nil
      } else {
        noticeIcon.image = UIImage(named: noticeIconPath)
      }
    }
  }
...
extension SettingUserNameTextField {
  ...
  func borderColor(from state: NicknameValidateState) -> CGColor {
    switch state {
    case .default:
  ...
  func noticeIconPath(from state: NicknameValidateState) -> String {
    switch state {
    case .duplicated, .overflow, .underflow:
