...
enum FeedViewControllerState {
  ...
  case initFeedNaviBarIsCheckedNotification(isCheckedNotification: Bool)
  
  /// NotificationViewController로 들어가게 된다면 영구저장소의 feed알림 확인 상태를 true(확인했다)로 변경해야 합니다.
  /// 또한 feedNaviBar의 updateIsCheckedNotification()를 통해 isChekcedNotification = true로 값을 전달해야 합니다.
  case goToNotification(isCheckedNotification: Bool)
  case goToPostSearch
